...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cfg=dict(GOOD,detail_strength=.4)
        path.write_text(json.dumps(dict(cfg,**_payload(cfg))))
        assert settings_io.load_config(path)['detail_strength']==.4
    ...
    pygame.init()
    try:
        menu=build();menu.set_state({'detail_strength':.4,'lang':'ru'});paint(menu)
        ...
        with patch.object(settings_io,'save_menu_layout') as save:
            commands.apply_menu_action(st,('detail_strength',.6))
            assert st.cfg['detail_strength']==.6;save.assert_called_once_with(st)
    finally:pygame.quit()
